...
class TestTrain(unittest.TestCase):
    def test_time_as_feature_equals_false(self):
        ...
        self.assertAlmostEqual(logits.sum().detach().numpy(),logits_sum_reference)

        pts_sum_reference = np.array(1.1808, dtype=np.float32)
        self.assertAlmostEqual(pts.sum().detach().numpy(),pts_sum_reference)
    ...
    def test_add_time_feature_to_input(self):
        ...
        self.assertEqual(x_expanded.sum().detach().numpy(), 22.5)
